...
class maskapai {
    /*
     * Greggy Gianini Firmansyah 1741720088 1FamilyNyimenk // D-IV Teknik
    ...
    static void metodeTransfer(int kode_bank, double total_biaya_fix) {
        ...
        System.out.println(" Silahkan transfer ke rekening " + bank + " dengan nomor " + rekening
                + " a/n Greggy Gianini Firmansyah sebesar Rp." + total_biaya_fix + " dalam waktu 1x24 jam");
    }
    ...
    public static void main(String args[]) {
        ...
        String[][] daftar_tujuan = {
                { "Malang - Surabaya \t( SBY - Juanda )", "Malang - Yogyakarta ( JOG - Adisutjipto )",
                        "Malang - Jakarta \t( CGK - Soekarno Hatta )" },
                { "Malang - Semarang \t( SRG - Achmad Yani )", "Malang - Bandung \t( BDO - Husein Sastranegara )",
                        "Malang - Palembang \t( PLM - Sultan Mahmud Badaruddin II )" },
                { "Malang - Denpasar \t( DPS - Ngurah Rai )", "Malang - Makassar \t( UPG - Hasanuddin )",
                        "Malang - Medan \t( KNO - Kuala Namu )" } };
        ...
        if (pilihan_tiket == 2) // Jika pilihan tiket yang dipilih itu Pulang-Pergi maka membutuhkan tanggal
                                // kembali
        {
        ...
        if (pilihan_tiket == 1 || pilihan_tiket == 2) {
            ...
            if (jumlah_total <= 0 || jumlah_tiket_dewasa < 0 || jumlah_tiket_anak < 0 || jumlah_tiket_bayi < 0) {
                ...
                System.out
                        .println(" -- JUMLAH TIKET MELEBIHI 7 ! JUMLAH TIKET SAAT INI ADALAH " + jumlah_total + " --");
            } else {
                System.out.println(" --------------------------------------------------");
                ...
                pilihan_tujuan = daftar_tujuan[maskapai - 1][tujuan - 1]; // Memilih tujuan berdasarkan array
                                                                          // daftar_tujuan
        ...
        if (tujuan == 1 || tujuan == 2 || tujuan == 3) {
            ...
            System.out.println(" ---------------------------------------------------------------------------");
            System.out.println(" Dewasa \tx " + jumlah_tiket_dewasa + " \t= Rp." + harga + " \tx " + jumlah_tiket_dewasa
                    + " \t= " + "Rp." + harga * jumlah_tiket_dewasa);

            if (jumlah_tiket_anak > 0) {
                System.out.println(" Anak-Anak \tx " + jumlah_tiket_anak + " \t= Rp." + (0.5 * harga) + " \tx "
                        + jumlah_tiket_anak + " \t= " + "Rp." + (0.5 * harga) * jumlah_tiket_anak);
            }

            if (jumlah_tiket_bayi > 0) {
                System.out.println(" Bayi \t\tx " + jumlah_tiket_bayi + " \t= Rp." + (0 * harga) + " \t\tx "
                        + jumlah_tiket_bayi + " \t= " + "Rp." + (0 * harga) * jumlah_tiket_bayi);
            }
        ...
        if (pilihan == 'y') {
            ...
            if (metode == 1) {
                ...
                System.out.println(" Tiket pesanan anda akan dikirimkan ke email " + email
                        + " dalam waktu 1x24jam. Terima kasih,enjoy the flight :)");
            }
        } else if (pilihan == 't') {
